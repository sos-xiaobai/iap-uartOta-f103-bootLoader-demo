...
void SendBit(unsigned char dat,unsigned char bitcnt)
{
 ...
 for(i=0;i<bitcnt;i++)
 {
     Ht1621_WR_0; /*置时钟线为高通知被控器开始接收数位*/

  if(( dat & 0x80 ) == 0)
  Ht1621_Data_0;
  else
  Ht1621_Data_1; /*发送数据由高位到低位传送*/

  Delay(1);
  Ht1621_WR_1; /*钳住总线准备下一个数据位*/
  Delay(1);
  dat=dat<<1;   /*发送数据左移一位*/
 }
}
...
void HT1621_Cmd_init(void)
{
 ...
 // HT1621_WriteCmd(Cmd8,WDTDIS); //WDT 溢出标志输出失效命令
  HT1621_WriteCmd(Cmd8,TIMERDIS ); //时基输出使能命令
 // HT1621_WriteCmd(Cmd8,RC256 ); ///系统时钟源片内RC振荡器命令
  HT1621_WriteCmd(Cmd8, BIAS ); //LCD1/3偏压选项4个公共口命令
  //HT1621_WriteCmd(Cmd9, F1 ); /*时基/WDT 时钟输出1Hz WDT设置为4 秒溢出*/
  //HT1621_WriteCmd(Cmd9, IRQEN ); /*时基/WDT 时钟输出1Hz WDT设置为4 秒溢出*/
}
...
void Time_Dis(unsigned char  min,unsigned char  second){
   ...
   dismem[2] = dismem[2]|tmp|0x80; //col常亮
...
void Pane_Dis(unsigned char t){
  ...
  switch(t){
     case 0:
          dismem[8]=dismem[8]|0x0E;dismem[9]=dismem[9]|0xFF;break;  //全亮
     case 1:
     ...
     default:
          dismem[8]=dismem[8]&0xF1;dismem[9]=dismem[9]&0x00;  //全灭 
          break;
  }
}
...
// uint8_t To_Bcd(unsigned char t)   //2位数字转BCD
// { 
//   unsigned char out;
//   if(t==0xFF) out=t;
