...
/*******************************************************************************
                      21. 附类型DP功能
********************************************************************************/
// #define         DP_WITH_TYPE_ENABLE  // 开启附类型DP

/*******************************************************************************
                      22. 记录型DP功能
********************************************************************************/
// #define       RECORD_TYPE_DP_ENABLE       // 开启记录型DP功能

/*******************************************************************************
                      23. 软总线服务
有可能需要根据接受数据大小适当调大接受数据缓冲区大小
********************************************************************************/
// #define SBUS_SERVICE_ENABLE // 开启软总线服务

/*******************************************************************************
                      24. 成品产测服务
********************************************************************************/
// #define PRODUCT_TEST_ENABLE // 开启成品测试
 ...
 * @brief  处理缓冲区溢出回调
 * 
 * 当接收到命令头，发现命令数据所需长度大于缓冲区长度时，调用该函数通知用户
 * 一般情况下这种情况不应该发生，
 * 如果在调试阶段，建议检查定义的缓冲区长度是否足够大
 * 如果在生产环境，建议直接复位MCU和模组，这可能预示潜在的攻击风险
 * 
 * 在此之后，会复位解析器和接收缓冲区
 * 
 * @param cmd 接收到的命令字（不可靠）
 * @param expected_len 命令声明的长度
 */
PROCESS_BUFFER_OVERFLOW_HANDLE_METHOD_E on_process_buffer_overflow(u8 cmd, u16 expected_len);

/**
 * @brief 缓冲区溢出后准备执行的第一条命令
 * 
 * 在 on_process_buffer_overflow 触发后，准备执行的第一条命令的回调
 * 在生产环境下，缓冲区溢出可能预示潜在的攻击，在此之后，我们无法判断这条命令真正来自模组还是上一条命令的data段
 * 入如果该命令试图执行危险操作（如固件升级），建议直接放弃
 * 
 * 如果返回false，在接收到下一条命令时，该回调会继续被调用，直到返回true为止
 * 
 * @param frame 命令帧指针
 * @param frame_len 命令帧长度
 * @return FALSE: 放弃这条命令 other: 处理这条命令
 */
